...
def show_puzzle(data, transparent=False, solid_color=None):
    min_x = min(x[0] for x in data['points'])
    max_x = max(x[0] for x in data['points'])
    min_y = min(x[1] for x in data['points'])
    max_y = max(x[1] for x in data['points'])
